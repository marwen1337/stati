...
export class Init1741116752873 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE "agent" ("id" varchar PRIMARY KEY NOT NULL, "name" varchar NOT NULL, "hashedAccessKey" varchar NOT NULL, "createdAt" datetime NOT NULL DEFAULT (datetime('now')), "updatedAt" datetime NOT NULL DEFAULT (datetime('now')))`
    )
    await queryRunner.query(
      `CREATE TABLE "result" ("id" varchar PRIMARY KEY NOT NULL, "status" varchar CHECK( "status" IN ('UP','DOWN') ) NOT NULL, "metrics" text NOT NULL, "createdAt" datetime NOT NULL DEFAULT (datetime('now')), "monitorId" varchar)`
    )
    await queryRunner.query(
      `CREATE INDEX "IDX_14acc619b27daaba4ed65955d0" ON "result" ("createdAt") `
    )
    await queryRunner.query(
      `CREATE TABLE "monitor" ("id" varchar PRIMARY KEY NOT NULL, "name" varchar NOT NULL, "intervalSeconds" integer NOT NULL, "type" varchar CHECK( "type" IN ('HTTP','CPU','PING','TCP') ) NOT NULL, "configuration" text NOT NULL, "createdAt" datetime NOT NULL DEFAULT (datetime('now')), "updatedAt" datetime NOT NULL DEFAULT (datetime('now')), "agentId" varchar)`
    )
    await queryRunner.query(`DROP INDEX "IDX_14acc619b27daaba4ed65955d0"`)
    await queryRunner.query(
      `CREATE TABLE "temporary_result" ("id" varchar PRIMARY KEY NOT NULL, "status" varchar CHECK( "status" IN ('UP','DOWN') ) NOT NULL, "metrics" text NOT NULL, "createdAt" datetime NOT NULL DEFAULT (datetime('now')), "monitorId" varchar, CONSTRAINT "FK_bdff545f19c78d3668b10cd0d35" FOREIGN KEY ("monitorId") REFERENCES "monitor" ("id") ON DELETE CASCADE ON UPDATE NO ACTION)`
    )
    await queryRunner.query(
